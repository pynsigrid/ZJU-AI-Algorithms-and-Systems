...
with open('./results/'+file+'.txt',"w") as f1:  
    ...
    for epoch in range(epochs):
        ...
        f1.write(str(loss.cpu().detach().numpy())+'\n')
        print('step:' + str(epoch + 1) + '/' + str(epochs) + ' || Total Loss: %.4f' % (loss))
        # print('{"metric":"loss", "loss": <int_or_float>, "epoch": <int>}')
        print('{{"metric": "loss", "value": {}}}'.format(loss))


torch.save(model.state_dict(), './results/'+file+'.pth')
print('Finish Training.')
plt.plot(loss_list,label = "loss")
plt.legend()
plt.savefig('./results/'+file+'.jpg')
# 4.评估模型，将自己认为最佳模型保存在 result 文件夹，其余模型备份在项目中其它文件夹，方便您加快测试通过。

print('val')
img = Image.open("test.jpg")
detector = FaceDetector()
recognize = Recognition(model_path='results/'+file+'.pth')
draw, all_num, mask_nums = recognize.mask_recognize(img)
# plt.show()
# plt.savefig('./result_img/'+file+'.jpg')
plt.imshow(draw)
draw.save('./results/'+file+'.png')  # PIL
