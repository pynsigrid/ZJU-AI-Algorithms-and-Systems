...
def my_search(maze):
    ...
    while True:
        ...
        if current_node.loc == maze.destination:  # 达到目标点
...
class Robot(QRobot):
    ...
    def sense_state(self):
        """
        重载sense_state函数，使其返回一个maze_size*maze_size维元组
        """
        state = [0 for i in range(self.maze.maze_size*self.maze.maze_size)]
        oldState = list(self.maze.sense_robot())
        state[oldState[0]*self.maze.maze_size + oldState[1]] = 1
    ...
    def _build_network(self):
        ...
        """build target model"""
        self.target_model = QNetwork(state_size=self.maze_size*self.maze_size, action_size=4, seed=seed).to(self.device)

        """build eval model"""
        self.eval_model = QNetwork(state_size=self.maze_size*self.maze_size, action_size=4, seed=seed).to(self.device)
